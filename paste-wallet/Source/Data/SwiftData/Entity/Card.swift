...
@Model
final class Card: Identifiable, Equatable, @unchecked Sendable {
  var id: UUID = UUID()
  var name: String = ""
  var issuer: String?
  ...
  static func previewItems() -> [Card] {
    return [
      Card(
        name: "ZERO Edition 2 1",
        issuer: "현대카드",
        brand: .visa,
        color: "#ffffff",
        number: [
          "fRA2PFBGYONOw8ZV73gujA==", "fRA2PFBGYONOw8ZV73gujA==", "fRA2PFBGYONOw8ZV73gujA==",
          "fRA2PFBGYONOw8ZV73gujA==",
        ],
        year: 28,
        month: 05,
        cvc: "x9PiMe/aOJ8Zilssnp5i9Q==",
        memo: "asdfhjhkasd\nasdf\nasdfasdf\nasdf\nasdf\n\nasdfasdfasdf"
      ),
      Card(
        name: "ZERO Edition 2 2",
        issuer: "현대카드",
        brand: .visa,
        color: "#ffffff",
        number: [
...
extension Card {
  ...
  static func fetchFavorite(modelContext: ModelContext) -> [Card] {
    ...
    let descriptor = FetchDescriptor<Card>(
      predicate: predicate,
      sortBy: [SortDescriptor(\.touch, order: .forward)]
    )
    do {
      return try modelContext.fetch(descriptor)
    } catch {
